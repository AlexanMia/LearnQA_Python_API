...
# Наша задача написать параметризованный тест.
...
# requests.get("https://playground.learnqa.ru/ajax/api/user_agent_check", headers={"User-Agent": "Some value here"})
...
data_provider = [("Mozilla/5.0 (Linux; U; Android 4.0.2; en-us; Galaxy Nexus Build/ICL53F) "
...
def test_user_agent(user_agent, expected_values):
    ...
    assert expected_values.items() <= response.json().items(), f'{expected_values.items()} != {response.json().items()}'
